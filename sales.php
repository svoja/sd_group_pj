...
<html lang="en">
<?php
$pageTitle = 'Sales Ledger | ARAII MOTO';
    ...
    <main class="max-w-[1400px] mx-auto py-20 px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-1 anim-load" style="animation-delay: 0.1s;">
                <div class="bg-white border border-obsidian-edge p-8 relative overflow-hidden shadow-2xl shadow-black/10">
                    ...
                    <form action="actions/create_order.php" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">Ref ID</label>
                                <div class="w-full bg-white/50 border border-obsidian-edge px-4 py-3 text-sm font-mono text-obsidian-muted uppercase cursor-not-allowed text-center">
                                    [ AUTO ]
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">PO Ref</label>
                                <div class="w-full bg-white/50 border border-obsidian-edge px-4 py-3 text-sm font-mono text-obsidian-muted uppercase cursor-not-allowed text-center">
                                    [ AUTO ]
                                </div>
                            </div>
                        ...
                        <div>
                            <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">Select Client Entity</label>
                            <select name="customer_id" required class="w-full bg-white border border-obsidian-edge px-4 py-3 text-sm font-mono focus:outline-none focus:border-premium transition-colors text-black appearance-none">
                        ...
                        <div>
                            ...
                            <select name="employee_id" required
                                class="w-full bg-white border border-obsidian-edge px-4 py-3 text-sm font-mono focus:outline-none focus:border-premium transition-colors text-black appearance-none">
                        ...
                        <div>
                            <div class="flex justify-between items-end mb-2">
                                <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted font-bold">Special Discount (USD Base)</label>
                                <span class="text-sm font-mono text-obsidian-muted uppercase tracking-widest">
                                    Format: <span class="text-black"><?= $_SESSION['currency'] ?></span>
                                </span>
                            </div>
            ...
            <div class="lg:col-span-2 anim-load" style="animation-delay: 0.2s;">
                <div class="bg-white border border-obsidian-edge overflow-hidden h-full flex flex-col shadow-2xl shadow-black/10">
                    <div class="px-8 py-6 border-b border-obsidian-edge bg-white/[0.02] flex justify-between items-center">
                        ...
                        <span class="text-premium font-mono text-sm animate-pulse">[ AWAITING_TRANSACTIONS ]</span>
                    </div>
                            ...
                            <tbody class="divide-y divide-obsidian-edge">
                                ...
                                <tr class="group hover:bg-white/[0.01] transition-colors <?= $itemCount === 0 ? 'opacity-60' : '' ?>">
                                    ...
                                    <td class="px-6 py-5">
                                        ...
                                        <div class="text-sm font-bold uppercase text-black mt-1">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="text-sm font-bold uppercase text-black">
                                            <?= htmlspecialchars($order['contact_name']) ?>
                                        </div>
                                        <div class="text-sm font-mono text-obsidian-muted">
                                    ...
                                    <td class="px-6 py-5 font-mono text-sm">
                                        <div class="text-black mb-2 border-b border-obsidian-edge pb-1">
                                            TOTAL: <span class="text-premium font-bold"><?= formatCurrency($order['total_amount']) ?></span>
                                        </div>
                                        <div class="text-sm text-obsidian-muted flex flex-col gap-1">
                                            <div>ITEMS: <span class="<?= $itemCount > 0 ? 'text-premium' : 'text-red-500' ?>"><?= $itemCount ?></span></div>
                                            
                                            <?php if ($order['membership_discount'] > 0): ?>
                                                <div class="text-premium">MEM (<?= htmlspecialchars($order['membership_level']) ?>): -<?= formatCurrency($order['membership_discount']) ?></div>
                                            <?php else: ?>
                                    ...
                                    <td class="px-6 py-5 text-right">
                                        <div class="flex justify-end items-center gap-4 opacity-50 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <a href="order_terminal.php?id=<?= $order['order_id'] ?>" 
                                               class="inline-block px-4 py-2 border <?= $itemCount === 0 ? 'border-red-600 text-red-600 hover:bg-red-600 hover:text-black animate-pulse' : 'border-premium text-premium hover:bg-premium hover:text-white' ?> text-sm font-black uppercase tracking-widest transition-colors">
                                ...
                                <?php endwhile; else: ?>
                                <tr>
                                    <td colspan="5" class="px-6 py-10 text-center text-obsidian-muted font-mono text-sm">No active purchase orders found.</td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
