...
<html lang="en">
<?php
$pageTitle = 'Invoice Generator | ARAII MOTO';
include 'partials/head.php';
?>

<body class="page-bg-cinematic bg-white text-black font-sans min-h-screen flex flex-col selection:bg-premium selection:text-white">
    ...
    <main class="flex-grow max-w-[1400px] mx-auto w-full py-10 px-8 flex flex-col gap-10">
        ...
        <?php if (isset($_GET['status'])): ?>
            <div class="p-4 bg-white border-l-4 border-premium font-mono text-sm uppercase tracking-widest anim-load shadow-2xl shadow-black/10">
                <?php 
        ...
        <div class="flex flex-col lg:flex-row gap-10">
            <div class="lg:w-1/3 flex flex-col gap-6 anim-load" style="animation-delay: 0.1s;">
                <div class="bg-white border border-obsidian-edge p-6 relative flex-grow shadow-2xl shadow-black/10">
                    <div class="absolute top-0 right-0 p-2 font-mono text-sm text-premium/30 uppercase tracking-widest">Sys_Op: <?= htmlspecialchars($code) ?></div>
                    ...
                    <form method="GET" action="invoices.php" class="mb-6 border-b border-obsidian-edge pb-6">
                        <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">Target P.O. Reference</label>
                        <select name="order_id" onchange="this.form.submit()" class="w-full bg-white border border-obsidian-edge px-4 py-3 text-sm font-mono focus:outline-none focus:border-premium text-black appearance-none">
                    ...
                    <form action="actions/create_invoice.php" method="POST" class="space-y-4 <?= !$previewOrder ? 'opacity-30 pointer-events-none' : '' ?>">
                        ...
                        <div>
                            ...
                            <input type="date" name="invoice_date" value="<?= date('Y-m-d') ?>" required 
                                   class="w-full bg-white border border-obsidian-edge px-4 py-3 text-sm font-mono focus:outline-none focus:border-premium text-black">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">Payment Status</label>
            ...
            <div class="lg:w-2/3 flex flex-col anim-load" style="animation-delay: 0.2s;">
                <div class="bg-white border border-obsidian-edge flex-grow flex flex-col shadow-2xl shadow-black/10">
                    <div class="px-8 py-6 border-b border-obsidian-edge bg-white/[0.02] flex justify-between items-center">
                        ...
                        <span class="text-premium font-mono text-sm animate-pulse">[ <?= $previewOrder ? 'DATA_LOCKED' : 'AWAITING_SELECTION' ?> ]</span>
                    </div>

                    <?php if ($previewOrder): ?>
                        <div class="p-6 border-b border-obsidian-edge bg-white/50 grid grid-cols-3 gap-4">
                            <div>
                                <p class="text-sm uppercase tracking-widest text-obsidian-muted mb-1">Client Entity</p>
                                <p class="font-bold uppercase"><?= htmlspecialchars($previewOrder['contact_name']) ?></p>
                                <p class="text-sm font-mono text-premium mt-1"><?= htmlspecialchars($previewOrder['customer_code']) ?></p>
                            </div>
                            <div>
                                <p class="text-sm uppercase tracking-widest text-obsidian-muted mb-1">P.O. Reference</p>
                                <p class="font-bold uppercase text-black"><?= htmlspecialchars($previewOrder['po_reference']) ?></p>
                                <p class="text-sm font-mono text-obsidian-muted mt-1"><?= date('d M Y', strtotime($previewOrder['order_date'])) ?></p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm uppercase tracking-widest text-obsidian-muted mb-1">Final Authorization Math</p>
                                <p class="text-2xl font-black text-black"><?= formatCurrency($previewOrder['total_amount']) ?></p>
                            </div>
                        </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-obsidian-edge">
                                    ...
                                    <tr class="hover:bg-white/[0.02] transition-colors">
                                        <td class="px-4 py-4">
                                            <div class="text-sm font-mono text-premium uppercase tracking-widest"><?= htmlspecialchars($item['product_code']) ?></div>
                                            <div class="text-sm font-bold uppercase text-black"><?= htmlspecialchars($item['product_name']) ?></div>
                                        </td>
                                        <td class="px-4 py-4 text-center font-mono text-sm"><?= $item['quantity'] ?></td>
                                        <td class="px-4 py-4 text-right font-mono text-sm text-obsidian-muted"><?= formatCurrency($item['unit_price']) ?></td>
                                        <td class="px-4 py-4 text-right font-mono text-sm font-bold text-black"><?= formatCurrency($item['total_price']) ?></td>
                                    </tr>
                                    <?php endwhile; ?>
                                </tbody>
                            </table>
                    ...
                    <?php else: ?>
                        <div class="flex-grow flex items-center justify-center p-10 text-center">
                            <p class="text-obsidian-muted font-mono text-sm uppercase tracking-[0.3em]">Select a Purchase Order to view manifest data.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="anim-load" style="animation-delay: 0.3s;">
            <div class="bg-white border border-obsidian-edge flex flex-col shadow-2xl shadow-black/10">
                <div class="px-8 py-6 border-b border-obsidian-edge bg-white/[0.02] flex justify-between items-center">
                    <h2 class="text-xl font-black uppercase tracking-tighter">Invoice_Ledger</h2>
                    <span class="text-obsidian-muted font-mono text-sm uppercase tracking-widest">[ HISTORICAL_RECORDS ]</span>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-obsidian-edge">
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                <td class="px-4 py-4">
                                    <div class="text-sm font-mono text-premium uppercase tracking-widest"><?= htmlspecialchars($inv['invoice_reference']) ?></div>
                                    <div class="text-sm font-mono text-obsidian-muted mt-1"><?= date('d M Y', strtotime($inv['invoice_date'])) ?></div>
                                </td>
                                
                                <td class="px-4 py-4">
                                    <div class="text-sm font-bold uppercase text-black"><?= htmlspecialchars($inv['contact_name']) ?></div>
                                    <div class="text-sm font-mono text-obsidian-muted mt-1">PO: <?= htmlspecialchars($inv['po_reference']) ?></div>
                                </td>
                                
                                <td class="px-4 py-4 text-right font-mono">
                                    <div class="text-sm font-bold text-black"><?= formatCurrency($inv['total_amount']) ?></div>
                                    <div class="text-sm text-obsidian-muted uppercase tracking-widest mt-1">[ <?= htmlspecialchars($inv['payment_method']) ?> ]</div>
                                </td>
                                ...
                                <td class="px-4 py-4 text-right">
                                    <div class="flex justify-end gap-4 opacity-50 group-hover:opacity-100 transition-opacity">
                                        <a href="print_invoice.php?id=<?= $inv['invoice_id'] ?>" target="_blank" class="text-sm font-black uppercase tracking-widest hover:text-black transition-colors">
                                            Print
                                        </a>
                                        
                                        <a href="edit_invoice.php?id=<?= $inv['invoice_id'] ?>" class="text-sm font-black uppercase tracking-widest hover:text-black transition-colors">
                                            Edit
                                        </a>
                                        ...
                                        <a href="actions/delete_invoice.php?id=<?= $inv['invoice_id'] ?>" 
                                           ...
                                           class="text-sm font-black uppercase tracking-widest text-premium hover:underline transition-colors">
                                           Drop
                                        </a>
                                    </div>
                                </td>
                            ...
                            <?php endwhile; else: ?>
                            <tr>
                                <td colspan="5" class="px-4 py-10 text-center text-obsidian-muted font-mono text-sm">No invoices found in ledger.</td>
                            </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
