...
// 3. Set variables to use in your ARAII MOTO header
...
<html lang="en">
<?php
$pageTitle = 'Personnel Management | ARAII MOTO';
...
<body class="page-bg-cinematic bg-white text-black font-sans min-h-screen overflow-x-hidden selection:bg-premium selection:text-white">
    ...
    <main class="max-w-[1400px] mx-auto py-20 px-8">
        ...
        <?php if (isset($_GET['status'])): ?>
            <div class="mb-6 p-4 bg-white border-l-4 border-premium font-mono text-sm uppercase tracking-widest anim-load shadow-lg shadow-black/10">
                <?php 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-1 anim-load" style="animation-delay: 0.1s;">
                <div class="bg-white border border-obsidian-edge p-8 relative overflow-hidden shadow-2xl shadow-black/10">
                    <div class="absolute top-0 right-0 p-2 font-mono text-sm text-premium/30 uppercase tracking-widest">System_Access: Alpha</div>
                    ...
                    <form action="actions/add_employee.php" method="POST" class="space-y-6">
                        <div>
                            <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">Full Identity</label>
                            <input type="text" name="emp_name" required placeholder="e.g. KENJI ARAII" 
                        ...
                        <div>
                            <label class="block text-sm uppercase tracking-[0.2em] text-obsidian-muted mb-2 font-bold">Tactical Position</label>
            ...
            <div class="lg:col-span-2 anim-load" style="animation-delay: 0.2s;">
                <div class="bg-white border border-obsidian-edge overflow-hidden shadow-2xl shadow-black/10">
                    <div class="px-8 py-6 border-b border-obsidian-edge bg-white/[0.02] flex justify-between items-center">
                        ...
                        <span class="text-premium font-mono text-sm animate-pulse">[ DATA_SECURE ]</span>
                    </div>
                            ...
                            <tbody class="divide-y divide-obsidian-edge">
                                ...
                                <tr class="group hover:bg-white/[0.01] transition-colors">
                                    <td class="px-8 py-5">
                                        ...
                                        <div class="text-sm font-mono text-obsidian-muted">UID: #<?= str_pad($emp['employee_id'], 3, '0', STR_PAD_LEFT) ?></div>
                                    </td>
                                    <td class="px-8 py-5 text-sm font-mono uppercase text-obsidian-muted">
                                        [ <?= htmlspecialchars($emp['position']) ?> ]
                                    </td>

                                    <td class="px-8 py-5">
                                    <?php if($isActive): ?>
                                        <div class="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-premium">
                                            <span class="relative flex h-1.5 w-1.5">
                                                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-premium opacity-75"></span>
                                                <span class="relative inline-flex rounded-full h-1.5 w-1.5 bg-premium"></span>
                                            </span>
                                            <?= $statusText ?>
                                        </div>
                                    <?php else: ?>
                                        <div class="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-red-600">
                                    ...
                                    <td class="px-8 py-5 text-right">
                                        <div class="flex justify-end gap-4">
                                            <?php if($isActive): ?>
                                                <a href="edit_employee.php?id=<?= $emp['employee_id'] ?>" class="text-sm font-black uppercase tracking-widest text-black/50 hover:text-black transition-colors">Edit</a>
                                                <a href="actions/soft_delete.php?id=<?= $emp['employee_id'] ?>" 
                                                onclick="return confirm('Decommission this entity?')"
                                                class="text-sm font-black uppercase tracking-widest text-premium hover:underline transition-colors">Drop</a>
                                            <?php else: ?>
                                                <span class="text-sm font-black uppercase tracking-widest text-obsidian-muted italic">System_Locked</span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                ...
                                <?php endwhile; else: ?>
                                <tr>
                                    <td colspan="3" class="px-8 py-10 text-center text-obsidian-muted font-mono text-sm">No active personnel found.</td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
